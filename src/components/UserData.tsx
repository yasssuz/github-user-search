...
export default function UserData({ user }: UserDataProps) {
  ...
  return (
    <Container>
      ...
      <LinksList>
        <LinkItem className={`${!user.location && "unavailable"}`}>
        ...
        <LinkItem className={`${!user.blog && "unavailable"}`}>
        ...
        <LinkItem className={`${!user.twitter && "unavailable"}`}>
        ...
        <LinkItem className={`${!user.company && "unavailable"}`}>
          <svg height='20' width='20' xmlns='http://www.w3.org/2000/svg'>
            <g fill={lightMode ? "#4b6a9b" : "#fff"}>
...
const StatsCardList = styled.div`
  border-radius: 1rem;
  list-style: none;
  display: flex;
  align-items: center;
